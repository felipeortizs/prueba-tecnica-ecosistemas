...
df_summary = df.groupby(['commerce_name', 'month', 'commerce_nit', 'commerce_email']).agg(
...
df_summary['commission_amount'] = df_summary.apply(calcular_comision, axis=1)

# Calcular IVA y valor total
df_summary['valor_iva'] = df_summary['commission_amount'] * 0.19
df_summary['valor_total'] = df_summary['commission_amount'] + df_summary['valor_iva']
...
df_summary['VALOR_COMISION'] = df_summary['VALOR_COMISION'].map('{:,.2f}'.format)
df_summary['VALOR_IVA'] = df_summary['VALOR_IVA'].map('{:,.2f}'.format)
df_summary['VALOR_TOTAL'] = df_summary['VALOR_TOTAL'].map('{:,.2f}'.format)
...
for nombre, group in df_summary.groupby('NOMBRE'):
